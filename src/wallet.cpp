...
// cashhcoin: optional setting to unlock wallet for block minting only;
...
// cashhcoin: total coins staked (non-spendable until maturity)
...
bool CWallet::SelectCoinsMinConf(int64 nTargetValue, unsigned int nSpendTime, int nConfMine, int nConfTheirs, vector<COutput> vCoins, set<pair<const CWalletTx*,unsigned int> >& setCoinsRet, int64& nValueRet) const
{
    ...
    BOOST_FOREACH(COutput output, vCoins)
    {
        ...
        if (pcoin->nTime > nSpendTime)
            continue;  // cashhcoin: timestamp must not exceed spend time
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            while(true)
            {
                ...
                // cashhcoin: sub-cent change is moved to fee
...
// cashhcoin: create coin stake transaction
...
// cashhcoin: check 'spent' consistency between wallet and txindex
// cashhcoin: fix wallet spent state according to txindex
...
// cashhcoin: disable transaction (only for coinstake)
